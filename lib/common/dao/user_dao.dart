...
import 'package:gsy_github_app_flutter/common/net/result_data1.dart';
...
class UserDao {
  ...
  static login(userName, password, store) async {
    ...
    var bytesPwd = utf8.encode(password);
    var base64StrPwd = base64.encode(bytesPwd);
    ...
    SignUpResponse resultData = null;
    if (res != null && res.result) {
      await LocalStorage.save(Config.PW_KEY, password);

       resultData = SignUpResponse.fromJson(res.data);
      if (Config.DEBUG) {
        print("user result " + resultData.code.toString());
        print(resultData.data);
        print(res.data.toString());
      }
      if(resultData.code==0){
        User dataUser = resultData.data != null ? new User.fromJson(resultData.data) : null;
        await LocalStorage.save(Config.API_TOKEN, dataUser.token);
        store.dispatch(new UpdateUserAction(dataUser));
      }
    }
    return new DataResult(resultData.data, res.result);
  }

  static signUp(name, password, store) async {
    ...
    await LocalStorage.save(Config.USER_NAME_KEY, name);
    await LocalStorage.save(Config.USER_BASIC_CODE, base64Str);
    ...
    var res = await httpManager.netFetch(Address.getSignup(),
        json.encode(requestParams), null, new Options(method: "post"));
    var resultData = null;
    if (res != null && res.result) {
      await LocalStorage.save(Config.PW_KEY, password);
