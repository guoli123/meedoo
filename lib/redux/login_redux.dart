...
final LoginReducer = combineReducers<bool>([
  ...
  TypedReducer<bool, SignUpSuccessAction>(_signUpResult),
...
bool _signUpResult(bool result, SignUpSuccessAction action) {
  if (action.success == true) {
...
class SignUpSuccessAction {
  ...
  SignUpSuccessAction(this.context, this.success);
}
...
class SignUpAction {
  ...
  SignUpAction(this.context, this.username, this.password);
}


Stream<dynamic> signUpEpic(Stream<dynamic> actions, EpicStore<GSYState> store) {
  Stream<dynamic> _signUp(
      SignUpAction action, EpicStore<GSYState> store) async* {
    CommonUtils.showLoadingDialog(action.context);
    var res = await UserDao.signUp(
        action.username.trim(), action.password.trim(), store);
    Navigator.pop(action.context);
    yield SignUpSuccessAction(action.context, (res != null && res.result));
  }
  return actions
      .whereType<SignUpAction>()
      .switchMap((action) => _signUp(action, store));
}
